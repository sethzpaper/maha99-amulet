...
## บันทึกสถานะล่าสุด 2026-05-30

สถานะหลังกลับมาไล่โปรเจกต์ใหม่:

- Supabase ตอนนี้พักไว้ก่อน เพราะโปรเจกต์ไม่แอคทีฟแล้ว
- เว็บยังใช้ React/Vite ได้ และ build ผ่าน
- Discord bot มี logic หลักและ SQLite แล้ว แต่เว็บยังไม่ได้ดึงข้อมูลจาก Discord/SQLite จริง
- หน้าเว็บถูกปรับให้ซ่อนเมนูหลักสำหรับคนที่ไม่ใช่ `admin` หรือ `super_admin` แล้ว
- ผู้ใช้ทั่วไปจะเห็นเฉพาะหน้าแจ้งว่าเมนูหลักเป็นของแอดมิน และปุ่มเข้าสู่ระบบ

สเต็ปถัดไปที่ควรทำก่อน Supabase:

1. เช็คชื่อ channel ใน Discord จริงให้ตรงกับแผน
2. เติม `.env` ของ bot เฉพาะค่า channel IDs และ token ที่จำเป็น
3. รัน bot แบบ local แล้วทดสอบ `/setup-video-channels` หรือ mapping channel
4. ทดสอบสร้างงานด้วย `/new-video-project` แล้วดูว่า SQLite บันทึก `video_projects`, `video_events` จริง
5. ค่อยเลือกว่าจะเชื่อมเว็บผ่าน Supabase, webhook API, หรืออ่านจากฐานกลางอื่น
